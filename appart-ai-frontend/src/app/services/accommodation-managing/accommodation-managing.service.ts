...
export class AccommodationManagingService {
  ...
  public getLandlordAccommodations(landlordId: string): Observable<Accommodation[]> {
    return this.getAuthHeaders().pipe(
      switchMap((headers) => this.http.get<Accommodation[]>(`${this.baseUrl}/landlord/${landlordId}`, { headers })),
  ...
  public getImagesAssociatedToAccommodation(accommodationId: string): Observable<ImageUrl[]> {
    return this.getAuthHeaders().pipe(
      switchMap((headers) => this.http.get<ImageUrl[]>(`${this.baseUrl}/${accommodationId}/images`, { headers })),
      catchError(this.handleError)
    );
  }

  public updateAccommodation(accommodationId: string, accommodation: Accommodation): Observable<Accommodation> {
    return this.getAuthHeaders().pipe(
      switchMap((headers) => this.http.put<Accommodation>(`${this.baseUrl}/${accommodationId}`, accommodation, { headers })),
      catchError(this.handleError)
    );
  }
  ...
  public deleteAccommodation(accommodationId: string): Observable<void> {
    return this.getAuthHeaders().pipe(
      switchMap((headers) => this.http.delete<void>(`${this.baseUrl}/${accommodationId}`, { headers })),
      catchError(this.handleError)
    );
  }
  ...
  public getAccommodationViewsAndInterests(accommodationId: string): Observable<Accommodation> {
    return this.getAuthHeaders().pipe(
      switchMap((headers) => this.http.get<Accommodation>(`${this.baseUrl}/${accommodationId}/views-interests`, { headers })),
      catchError(this.handleError)
    );
  }

  public updateAccommodationAddress(accommodationId: string, address: Address): Observable<Accommodation> {
    return this.getAuthHeaders().pipe(
      switchMap((headers) => this.http.put<Accommodation>(`${this.baseUrl}/${accommodationId}/address`, address, { headers })),
      catchError(this.handleError)
    );
  }
